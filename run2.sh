...
while [ $valid_hash -eq 0 ]; do
    ...
    # 这里假设API通过GET请求验证hash，并返回状态码200表示有效
    response_code=$(curl -s -o /dev/null -w "%{http_code}" "https://api-test1.container1.titannet.io/api/v2/device/binding?hash=$user_hash")
